...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_server_misc($server)
  {
    ...
    $misc['icon_status_webkp']  = lgsl_icon_status_webkp($server['b']['status'], $server['s']['password'], $server['b']['pending']);
    ...
    $misc['text_badge_status']  = lgsl_text_badge_status($server['b']['status'], $server['s']['password'], $server['b']['pending']);
...
function lgsl_icon_status_webkp($status, $password, $pending = 0)
{
  global $lgsl_url_path;

  if ($pending)  { return "bg-secondary"; }
  if (!$status)  { return "bg-danger"; }
  if ($password) { return "bg-info"; }
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_text_status($status, $password, $pending = 0)
  {
    ...
    if ($password) { return 'Online with password'; }
...
function lgsl_text_badge_status($status, $password, $pending = 0)
{
  if ($pending)  { return '<span class="badge badge-secondary shadow-sm">PENDING</span>'; }
  if (!$status)  { return '<span class="badge badge-danger shadow-sm">NO RESPONSE</span>'; }
  if ($password) { return '<span class="badge badge-warning shadow-sm">ONLINE WITH PASSWORD</span>'; }

  return '<span class="badge badge-success shadow-sm">ONLINE</span>';
}
